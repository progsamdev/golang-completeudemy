...
func GetEventById(id uuid.UUID) (*Event, error) {
	...
	err = preparedStatement.QueryRow(id).Scan(&event.Id, &event.Name, &event.Description, &event.Location, &event.DateTime, &event.UserID)
...
func (event *Event) Register(userId uuid.UUID) error {
	...
	_, err = preparedStatement.Exec(event.Id, userId)
	if err != nil {
		return err
	}
...
func (event *Event) Unregister(userId uuid.UUID) error {
